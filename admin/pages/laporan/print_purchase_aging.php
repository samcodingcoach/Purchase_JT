<?php
/**
 * Halaman Cetak Laporan Purchase Aging (Print / PDF)
 * Path: admin/pages/laporan/print_purchase_aging.php
...
$agingRange = trim($_GET['aging_range'] ?? '');
...
<head>
    ...
    <title>Laporan Purchase Aging - <?= htmlspecialchars($companyName) ?></title>
    <!-- Bootstrap CSS & Icons -->
    ...
    <div class="container d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div class="d-flex align-items-center gap-3">
            <span class="fw-bold fs-6">
                <i class="bi bi-printer text-info me-1"></i> Cetak Laporan Purchase Aging
            </span>
        </div>
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            <a href="<?= BASE_URL ?>/admin/pages/laporan/purchase_aging.php" class="btn btn-outline-light btn-sm px-3">
...
<div class="print-wrapper <?= !$useKop ? 'no-kop' : '' ?>">
    ...
    <div class="doc-title-box" style="text-align: center; margin: 16px 0 20px 0; border-bottom: none;">
        <div class="doc-title" style="font-size: 15px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0;">Laporan Purchase Order Aging (PO Aging)</div>
    </div>

    <!-- TABEL PURCHASE AGING -->
    <table class="table-items-main mb-4" id="tablePrintReport">
        <thead>
            <tr>
                <th class="text-center" style="width: 30px;">No</th>
                <th style="text-align: left; width: 110px;">Nomor PO</th>
                <th class="text-center" style="width: 80px;">Tgl PO</th>
                <th style="text-align: left;">Nama Vendor</th>
                <th style="text-align: left; width: 120px;">Pembuat PO</th>
                <th class="text-center" style="width: 130px;">Status</th>
                <th class="text-center" style="width: 110px;">Tgl Update</th>
                <th class="text-center" style="width: 85px;">Umur PO</th>
            </tr>
        </thead>
        ...
        <tfoot id="printTableFoot" style="display: none;">
            <tr class="total-row">
                <td colspan="7" class="text-center text-uppercase">Total Outstanding PO</td>
                <td class="text-center fw-bold" id="footTotalPo" style="font-size: 11px;">0 PO</td>
            </tr>
        </tfoot>
    </table>
        ...
        <div class="row">
            <div class="col-4 sig-col">
                ...
                <div class="sig-line-box">
                    <span class="sig-person-name">Staf Purchasing</span>
                </div>
            ...
            <div class="col-4 sig-col">
                ...
                <div class="sig-header-sub">&nbsp;</div>
                <div class="sig-line-box">
                    <span class="sig-person-name">Manager Purchasing</span>
                </div>
                <div class="sig-footer-note">Tanggal: ....................</div>
            </div>
            <div class="col-4 sig-col">
...
const AGING_RANGE = "<?= addslashes($agingRange) ?>";
...
function renderPrintTable(summary, grand) {
    ...
    if (!summary || summary.length === 0) {
        tbody.innerHTML = `<tr><td colspan="8" class="text-center py-3">Tidak ada data purchase aging pada filter ini.</td></tr>`;
    ...
    summary.forEach((item, idx) => {
        ...
        <tr>
            ...
            <td class="fw-semibold">${escapeHtml(item.nomor_po || '-')}</td>
            <td class="text-center">${item.formatted_tanggal_po || '-'}</td>
            <td>${escapeHtml(item.nama_perusahaan || '-')}</td>
            <td>${escapeHtml(item.pembuat_po || '-')}</td>
            <td class="text-center">${escapeHtml(item.status || '-')}</td>
            <td class="text-center">${item.formatted_tanggal_update || '-'}</td>
            <td class="text-center fw-bold">${item.umur_po} Hari</td>
        </tr>`;
    });
