<?php
/**
 * Halaman Cetak Laporan Realisasi Kuantitas (Print / PDF)
 * Path: admin/pages/laporan/print_realisasi_kuantitas.php
...
<head>
    ...
    <title>Laporan Realisasi Kuantitas - <?= htmlspecialchars($companyName) ?></title>
    <!-- Bootstrap CSS & Icons -->
    ...
    <div class="container d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div class="d-flex align-items-center gap-3">
            <span class="fw-bold fs-6">
                <i class="bi bi-printer text-info me-1"></i> Cetak Laporan Realisasi Kuantitas
            </span>
        </div>
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            <a href="<?= BASE_URL ?>/admin/pages/laporan/realisasi_kuantitas.php" class="btn btn-outline-light btn-sm px-3">
...
<div class="print-wrapper <?= !$useKop ? 'no-kop' : '' ?>">
    ...
    <div class="doc-title-box" style="text-align: center; margin: 16px 0 20px 0; border-bottom: none;">
        <div class="doc-title" style="font-size: 15px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0;">Laporan Realisasi Kuantitas</div>
    </div>

    <!-- TABEL REALISASI KUANTITAS -->
    <table class="table-items-main mb-4" id="tablePrintReport">
        <thead>
            <tr>
                <th class="text-center" style="width: 35px;">No</th>
                <th style="text-align: left;">Nama Vendor</th>
                <th class="text-end" style="width: 110px;">KTS PO</th>
                <th class="text-end" style="width: 130px;">KTS RCV</th>
                <th class="text-end" style="width: 130px;">KTS RTN</th>
                <th class="text-end" style="width: 120px;">Realisasi (%)</th>
            </tr>
        </thead>
        ...
        <tfoot id="printTableFoot" style="display: none;">
            <tr class="total-row">
                ...
                <td class="text-end" id="footQtyPo">0</td>
                <td class="text-end" id="footQtyRcv">0</td>
                <td class="text-end" id="footQtyRetur">0</td>
                <td class="text-end fw-bold" id="footPersen" style="font-size: 11px;">0.00%</td>
            </tr>
        </tfoot>
    </table>
        ...
        <div class="row">
            <div class="col-4 sig-col">
                ...
                <div class="sig-line-box">
                    <span class="sig-person-name">Staf Logistik / Purchasing</span>
                </div>
...
function renderPrintTable(summary, grand) {
    ...
    if (!summary || summary.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center py-3">Tidak ada data realisasi kuantitas pada filter ini.</td></tr>`;
    ...
    summary.forEach((item, idx) => {
        ...
        <tr>
            ...
            <td class="fw-semibold">${escapeHtml(item.nama_perusahaan || '-')}</td>
            <td class="text-end">${Number(item.qty_po || 0).toLocaleString('id-ID')}</td>
            <td class="text-end">${Number(item.qty_rcv || 0).toLocaleString('id-ID')}</td>
            <td class="text-end">${Number(item.qty_retur || 0).toLocaleString('id-ID')}</td>
            <td class="text-end fw-bold">${item.formatted_persentase || '0.00%'}</td>
        </tr>`;
    });
