<?php
/**
 * Halaman Cetak Laporan Pengeluaran Lain-lain (Print / PDF)
 * Path: admin/pages/laporan/print_pengeluaran_lain.php
...
// Periode Text
$periodeParts = [];
if (!empty($startDate)) $periodeParts[] = 'Mulai: ' . date('d/m/Y', strtotime($startDate));
if (!empty($endDate)) $periodeParts[] = 'Sampai: ' . date('d/m/Y', strtotime($endDate));
$periodeText = !empty($periodeParts) ? implode(' - ', $periodeParts) : 'SEMUA PERIODE';
if (!empty($kategori)) $periodeText .= " ({$kategori})";
...
<head>
    ...
    <title>Laporan Pengeluaran Lain-lain - <?= htmlspecialchars($companyName) ?></title>
    <!-- Bootstrap CSS & Icons -->
    ...
    <div class="container d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div class="d-flex align-items-center gap-3">
            <span class="fw-bold fs-6">
                <i class="bi bi-printer text-info me-1"></i> Cetak Laporan Pengeluaran Lain-lain
            </span>
        </div>
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            <a href="<?= BASE_URL ?>/admin/pages/laporan/pengeluaran_lain.php" class="btn btn-outline-light btn-sm px-3">
...
<div class="print-wrapper <?= !$useKop ? 'no-kop' : '' ?>">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h5 class="fw-bold text-dark mb-0">LAPORAN PENGELUARAN LAIN-LAIN</h5>
    ...
    <!-- TABEL DATA PENGELUARAN LAIN-LAIN -->
        ...
        <table class="table-pajak align-middle">
            <thead>
                <tr>
                    ...
                    <th style="width: 160px;" class="text-end">BIAYA</th>
                </tr>
            </thead>
            <tbody id="printTableBody">
                <tr>
                    <td colspan="5" class="text-center py-4 text-muted">
                        <div class="spinner-border spinner-border-sm text-primary me-2"></div> Memuat data pengeluaran lain-lain...
                    </td>
                </tr>
            </tbody>
            <tfoot>
                <tr class="fw-bold text-dark">
                    ...
                    <td class="text-end font-monospace py-2" id="printGrandBiaya">0</td>
                </tr>
            </tfoot>
        </table>
    </div>
...
async function loadPrintData() {
    ...
    try {
        const res = await fetch(`<?= BASE_URL ?>/api/laporan/pengeluaran_lain.php?${params.toString()}`);
...
function renderPrintRows(rows, totals) {
    ...
    if (!rows || rows.length === 0) {
        showPrintEmpty('Tidak ada data pengeluaran lain-lain pada periode ini.');
        return;
    }
    ...
    rows.forEach((r, idx) => {
        ...
        <tr>
            ...
            <td>
                <strong class="text-dark">${escapeHtml(r.keterangan)}</strong>
            </td>
            <td class="font-monospace fw-bold text-dark">
                ${escapeHtml(r.nomor_ref || '-')}
            </td>
            <td>
                <span class="font-monospace text-dark">${escapeHtml((r.tanggal_ref || '').split(' ')[0] || '-')}</span>
            </td>
            <td class="text-end font-monospace fw-bold text-dark">${formatAngka(r.biaya)}</td>
        </tr>`;
    });
    ...
    // Grand Totals
    if (totals) {
        document.getElementById('printGrandBiaya').textContent = formatAngka(totals.grand_total_biaya);
    }
}
