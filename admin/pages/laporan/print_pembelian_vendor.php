<?php
/**
 * Halaman Cetak Laporan Pembelian per Vendor (Print / PDF)
 * Path: admin/pages/laporan/print_pembelian_vendor.php
...
<head>
    ...
    <title>Laporan Pembelian per Vendor - <?= htmlspecialchars($companyName) ?></title>
    <!-- Bootstrap CSS & Icons -->
    ...
    <div class="container d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div class="d-flex align-items-center gap-3">
            <span class="fw-bold fs-6">
                <i class="bi bi-printer text-info me-1"></i> Cetak Laporan Pembelian per Vendor
            </span>
        </div>
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            <a href="<?= BASE_URL ?>/admin/pages/laporan/pembelian_vendor.php" class="btn btn-outline-light btn-sm px-3">
...
<div class="print-wrapper <?= !$useKop ? 'no-kop' : '' ?>">
    ...
    <div class="doc-title-box" style="text-align: center; margin: 16px 0 20px 0; border-bottom: none;">
        <div class="doc-title" style="font-size: 15px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0;">Laporan Pembelian per Vendor</div>
    </div>

    <!-- TABEL PEMBELIAN PER VENDOR -->
    <table class="table-items-main mb-4" id="tablePrintReport">
        <thead>
            <tr>
                <th class="text-center" style="width: 35px;">No</th>
                <th style="text-align: left;">Nama Vendor</th>
                <th class="text-end" style="width: 220px;">Total Terbayar</th>
                <th class="text-center" style="width: 180px;">Keterangan</th>
            </tr>
        </thead>
...
async function loadPrintData() {
    ...
    try {
        ...
        if (res.success && res.data) {
            ...
            tbody.innerHTML = `<tr><td colspan="4" class="text-center py-3">${res.message || 'Gagal memuat data.'}</td></tr>`;
        }
    } catch (err) {
        tbody.innerHTML = `<tr><td colspan="4" class="text-center py-3">Terjadi kesalahan: ${err.message}</td></tr>`;
    }
}
...
function renderPrintTable(summary, grand) {
    ...
    if (!summary || summary.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" class="text-center py-3">Tidak ada data pembelian per vendor pada filter ini.</td></tr>`;
    ...
    summary.forEach((item, idx) => {
        ...
        <tr>
            ...
            <td class="text-end fw-bold">${Number(item.total_terbayar || 0).toLocaleString('id-ID')}</td>
            <td class="text-center">${escapeHtml(item.keterangan || '-')}</td>
        </tr>`;
    });
