...
// Ambil kode_karyawan jika belum ada di session
$namaPembuat = $user['nama'] ?? $user['nama_lengkap'] ?? $user['username'] ?? 'Petugas';
$kodeKaryawan = $user['kode_karyawan'] ?? '';
if (empty($kodeKaryawan) && !empty($user['id_karyawan'])) {
    $stmtKry = $conn->prepare("SELECT kode_karyawan FROM karyawan WHERE id_karyawan = ? LIMIT 1");
    if ($stmtKry) {
        $stmtKry->bind_param("i", $user['id_karyawan']);
        $stmtKry->execute();
        $resKry = $stmtKry->get_result()->fetch_assoc();
        if ($resKry && !empty($resKry['kode_karyawan'])) {
            $kodeKaryawan = $resKry['kode_karyawan'];
        }
        $stmtKry->close();
    }
}
...
// Ambil daftar site
$sites = [];
$resSite = $conn->query("SELECT id_site, kode_site, nama_site, jenis_site FROM site ORDER BY nama_site ASC");
if ($resSite) {
    while ($row = $resSite->fetch_assoc()) {
        $sites[] = $row;
    }
}
                                ...
                                <div class="p-3 bg-light rounded-3 border h-100">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select form-select-sm fw-semibold" id="idSite" required onchange="handleSiteChange()">
                                            <option value="">-- Pilih Site Gudang --</option>
                                            <?php foreach ($sites as $st): ?>
                                                <option value="<?= $st['id_site'] ?>">
                                                    <?= htmlspecialchars($st['nama_site']) ?> (<?= htmlspecialchars($st['kode_site'] ?: 'SITE') ?>)
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
